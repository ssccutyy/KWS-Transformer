...
def data_to_file(image_data, label):
  with open('512_1875data.pkl', 'wb') as f:
...
def train(flags):
  ...
  # train_list = []
  # train_gt_list = []
  train_list, train_gt_list = file_to_data('512_1875data.pkl')
  # Training loop.
  for training_step in range(start_step, training_steps_max + 1):
    if training_step > 0:
      ...
      train_fingerprints = train_list[(training_step-1)%1875]
      train_ground_truth = train_gt_list[(training_step-1)%1875]
