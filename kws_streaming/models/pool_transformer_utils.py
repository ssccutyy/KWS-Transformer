...
class pooling(tf.keras.layers.Layer):
    ...
    def call(self, x):
        x = x[:, :, :, None]
        p = self.pool(x) - x
        output = tf.squeeze(p, 3)
        return output

class TransformerBlock(tf.keras.layers.Layer):
    def __init__(self, embed_dim, pool_size, ff_dim, dropout=0.1, prenorm=False, approximate_gelu=False):
        super(TransformerBlock, self).__init__()
        self.att = pooling(pool_size=pool_size)
...
class poolTransformer(tf.keras.Model):
    def __init__(
        self,
        num_patches,
        num_layers,
        ...
        self.enc_layers = [
            TransformerBlock(d_model, pool_size, mlp_dim, dropout, prenorm, approximate_gelu)
